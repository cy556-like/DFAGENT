...
# ===== 导出触发关键词（仅用于代码层快速匹配，不注入 prompt） =====
# 当用户输入包含对应智能体的关键词时，自动启用分步导出流程：
# Phase 1: 立即返回"正在生成文档..."（1-3秒）
# Phase 2: 后台独立 LLM 生成完整文档内容并导出（~30秒）
EXPORT_TRIGGER_KEYWORDS = {
    'dfmea-risk-agent': [
        'DFMEA', 'PFMEA', '失效', '风险', 'FMEA', 'AP法', '探测', '预防',
    ],
    'part-design-agent': [
        '参数化', '建模', '约束', 'PMI', '基准', '公差', '间隙', '过盈',
    ],
    'simulation-optimization-agent': [
        '仿真', '优化', 'DOE', '分析脚本', '多目标', '拓扑', '载荷',
    ],
    'material-selection-agent': [
        '材料', '轻量化', '选材', '替代', '减重', '以塑代钢',
    ],
    'manufacturing-process-agent': [
        '工艺卡', '冲压', '注塑', '焊接', '压铸', '热处理', '缺陷',
    ],
    'ee-design-agent': [
        'PCB', '电路', 'EMI', 'DDR', 'PDN', '阻抗', '布线',
    ],
    'embedded-software-agent': [
        '代码', 'MISRA', 'ASIL', 'SW-FMEA', '看门狗', '安全', 'HARA',
    ],
    'test-verification-agent': [
        '试验', '验证', 'DVP', '测试', 'PSN', '载荷谱', '疲劳',
    ],
...
# 导出格式映射：哪些智能体默认导出 Excel，哪些默认导出 Word
EXPORT_FORMAT_MAP = {
    'dfmea-risk-agent': 'xlsx',
    'part-design-agent': 'docx',
    'simulation-optimization-agent': 'docx',
    'material-selection-agent': 'xlsx',
    'manufacturing-process-agent': 'xlsx',
    'ee-design-agent': 'docx',
    'embedded-software-agent': 'docx',
    'test-verification-agent': 'xlsx',
    'equipment-production-agent': 'docx',
    'standards-innovation-agent': 'docx',
}
