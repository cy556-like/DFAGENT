...
- 按 SKILL.md 工作流执行：收集产品名/缺陷描述/客户等关键信息 → 匹配模板（涂装/装配/焊接/尺寸/通用）→ 生成单Sheet .xlsx + .docx

- 信息不足时用 AskUserQuestion 主动追问用户（产品名、缺陷描述、客户名必填）

- 8D 报告不走通用导出工具！确认参数后直接执行: python skills/8d-skill/scripts/generate_8d.py --product ... --output-dir data/export，将输出的文件路径转为下载链接返回。禁止先输出完整8D内容再导出，只需返回文件链接+简短摘要(5行内)
...
- 分析报告、评估报告、DFMEA/PFMEA表格、对比分析、方案比较、工艺卡、试验方案等专业内容 → **直接在对话中输出至少70%的完整内容（包含关键分析、表格、数据、结论），然后自动调用导出工具生成文件供用户下载。不要询问用户是否需要导出，直接给文件。**
...
- ✅ 正确流程示例：先输出"## DFMEA 分析\n\n| 序号 | ... |\n|---|---|\n| 1 | ... |\n\n（完整分析内容）..."→ 最后调用 export_xlsx_tool 生成文件 → 展示下载链接
